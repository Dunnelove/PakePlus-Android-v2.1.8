...
<body>
    ...
    <div id="hangar-guide" class="fixed inset-0 z-[100] flex items-center justify-center" style="display:none; background:rgba(0,0,0,0.8);">
        <div class="guide-box relative w-[90%] max-w-lg p-6 bg-black/95 border-2 border-yellow-500 rounded-lg shadow-[0_0_40px_rgba(255,215,0,0.4),inset_0_0_20px_rgba(255,215,0,0.1)]">
            <div class="guide-header text-center mb-4">
                <h2 class="text-2xl font-bold text-yellow-300 orbitron" style="text-shadow:0_0_10px_#ffd700">🔧 机库改造指南</h2>
                <p class="text-xs text-yellow-500 mt-1">提升战力的核心系统</p>
            </div>
            <div class="guide-content space-y-4 text-gray-300 text-sm leading-relaxed">
                <div class="bg-cyan-950/30 p-3 rounded border border-cyan-800/50">
                    <h3 class="text-cyan-400 font-bold mb-2 flex items-center gap-2">
                        <span class="text-lg">⚙️</span> 基础升级
                    </h3>
                    <p class="text-xs text-gray-400">使用金币永久提升机体属性：<span class="text-cyan-300">攻击力、生命值、护盾、移速</span>等。升级后所有模式通用，是提升战力的基础途径。</p>
                </div>
                <div class="bg-orange-950/30 p-3 rounded border border-orange-800/50">
                    <h3 class="text-orange-400 font-bold mb-2 flex items-center gap-2">
                        <span class="text-lg">🔫</span> 武器改装
                    </h3>
                    <p class="text-xs text-gray-400">为武器装配特殊效果：<span class="text-orange-300">燃烧、迟缓、破甲、虚弱</span>等。解锁后可升级至Lv.3，大幅增强战斗能力。</p>
                </div>
                <div class="bg-purple-950/30 p-3 rounded border border-purple-800/50">
                    <h3 class="text-purple-400 font-bold mb-2 flex items-center gap-2">
                        <span class="text-lg">⚡</span> 大招改装
                    </h3>
                    <p class="text-xs text-gray-400">解锁并强化终极技能：<span class="text-purple-300">喀秋莎火箭、黑洞、雷暴、狂暴、无敌</span>。每种大招都有独特效果，升级后威力倍增！</p>
                </div>
                <div class="bg-yellow-950/20 p-2 rounded border border-yellow-800/30 text-center">
                    <p class="text-xs text-yellow-400">💡 提示：优先升级攻击力和生命值，再考虑改装系统</p>
                </div>
            </div>
            <div class="guide-nav flex justify-center mt-6">
                <button id="hangar-guide-close" class="btn py-2 px-8 text-sm !border-yellow-500 !text-yellow-300 hover:!bg-yellow-500 hover:!text-black" onclick="closeHangarGuide()">开始升级</button>
            </div>
        </div>
    </div>
...
const TEXTS = {
    ...
    pas_speed: "引擎过载", desc_speed: "移速+10%，敌人减速5%/级，敌方弹幕减速5%/级。",
...
if (saveData.firstHangarGuide === undefined) { saveData.firstHangarGuide = true; }
    ...
    activeProjectiles.forEach(p => {
        ...
        if(p.team === 'player') {
            ...
            if (p.type === 'enemy_laser') {
                ...
                p.x+=p.vx*spd*enemyBulletSlowFactor; p.y+=p.vy*spd*enemyBulletSlowFactor; p.life-=spd;
                if(p.life<=0) p.marked=true;
                let dx = player.x - p.x; let dy = player.y - p.y;
                ...
                p.x += p.vx * spd * enemyBulletSlowFactor; p.y += p.vy * spd * enemyBulletSlowFactor;
                ...
                if (p.phase === 'tracking') {
                    ...
                    p.x += p.vx * spd * enemyBulletSlowFactor;
                    p.y += p.vy * spd * enemyBulletSlowFactor;
...
function showShop() { 
    if (saveData.currentRun) return notifyUnlock("任务进行中，无法升级机库。"); 
    document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active')); 
    document.getElementById('shop-screen').classList.add('active'); 
    renderShop(); 
    
    if (saveData.firstHangarGuide) {
        showHangarGuide();
    }
}

function showHangarGuide() {
    document.getElementById('hangar-guide').style.display = 'flex';
    AudioSys.play('ui_click');
}

function closeHangarGuide() {
    document.getElementById('hangar-guide').style.display = 'none';
    saveData.firstHangarGuide = false;
    saveGame();
    AudioSys.play('ui_click');
}
